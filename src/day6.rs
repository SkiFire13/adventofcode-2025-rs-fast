...
unsafe fn part2_inner(input: &str) -> i64 {
    ...
    while offset != len + 1 {
        ...
        let lane = u32x4::from_array([v1, v2, v3, v4]);
        let lane = std::mem::transmute::<_, u8x16>(lane);
        let lane = simd_swizzle!(lane, [0, 4, 8, 12, 1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15]);

        let len_mask = std::mem::transmute::<_, u32x4>(lane & u8x16::splat(0b00010000))
            .simd_eq(u32x4::splat(0))
            .to_bitmask();
        let len = (len_mask | 0b10000).trailing_zeros() as usize;

        let mask = (lane & u8x16::splat(0b00010000)).simd_eq(u8x16::splat(0));

        let shift = std::mem::transmute::<_, u32x4>(mask.to_int()).leading_ones();
        let lane = std::mem::transmute::<_, u32x4>(lane) << shift;
        let lane = std::mem::transmute::<_, u8x16>(lane);
        ...
        let mask = mask32x4::from_bitmask((len_mask as u64).wrapping_sub(1));

        if *ptr_o.add(offset) == b'+' {
            let lane = mask.select(lane, u32x4::splat(0));
            tot_acc += lane;
        } else {
            let lane = mask.select(lane, u32x4::splat(1));
            tot += (lane.extract::<0, 2>() * lane.extract::<2, 2>())
                .cast::<u64>()
                .reduce_product();
        }
